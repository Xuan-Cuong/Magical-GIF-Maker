...
// Fix for gifenc module declaration
// Create a custom declaration file for gifenc instead of inline declaration.
...
const uploadedImages: string[] = []; // Store uploaded image URLs

// Handle image upload
const imageUploadInput = document.getElementById('image-upload') as HTMLInputElement;
if (imageUploadInput) {
  // Add delete button functionality for uploaded images
  const uploadedImagesContainer = document.createElement('div');
  uploadedImagesContainer.id = 'uploaded-images-container';
  uploadedImagesContainer.style.display = 'flex';
  uploadedImagesContainer.style.flexWrap = 'wrap';
  uploadedImagesContainer.style.gap = '10px';
  imageUploadInput?.parentElement?.appendChild(uploadedImagesContainer);
...
// Update the `run` function to handle API errors and log request/response details
async function run(value: string) {
  ...
  try {
    // Include uploaded images as references in the prompt
    const uploadedImagesPrompt = uploadedImages.length
      ? `The following images are provided as visual references: ${uploadedImages.map((_, index) => `Image ${index + 1}`).join(', ')}.`
      : '';

    const combinedPrompt = `${value} ${uploadedImagesPrompt}`;

    console.log('Sending prompt to API:', combinedPrompt);
...
**Key Constraints:** No racial labels. Neutral skin tone descriptors when included. Cartoonish/doodle style always implied, especially for people. One text display method only.`,
    ...
    console.log('Expand prompt response:', expandPromptResponse);
...
**Reference Images:** ${uploadedImages.join(', ')}`,
      config: {
        temperature: 1,
    ...
    console.log('Content stream response:', response);

    const images = [];
    let frameCount = 0;
    const maxFrames = 10; // Maximum number of frames

    for await (const chunk of response) {
      if (chunk.candidates && chunk.candidates[0].content?.parts) {
        for (const part of chunk.candidates[0].content.parts) {
          if (frameCount >= maxFrames) break; // Stop if maximum frames reached
    ...
    if (frameCount < 2 && images.length < 2) {
    ...
    updateStatus(`Error generating animation: ${error}`);
    return false;
  } finally {
    if (generateButton) {
